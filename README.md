...
- **サンプルデータで即体験** — JSONL がなくても「サンプルデータで試してみる」ボタンで 3 プロジェクト × 10 セッションのデモを即座に表示
...
- **Tips データベース（21 件）** — zenn / note / Anthropic 公式 / DEV Community / MindStudio などの実測値・公式情報に基づく削減 Tips（`.claudeignore`、Planモード、Caveman Mode、サブエージェント委託、CLAUDE.md 3層構造など）を出典付きで内蔵。ユーザーの実データと条件マッチして優先度順に表示
...
- **診断書画像のダウンロード** — 分析サマリーを 1 枚の PNG（ダーク + シアンのターミナル風レポート）として保存
- **X シェアボタン** —
  - **テキストのみ**: ROI / 最大の無駄パターン / 丁寧表現回数を含むコンパクトな文面を自動生成
  - **診断書付きでシェア**: PNG をクリップボードにコピー → 小窓ポップアップで X の作成画面を開く → `Ctrl+V` で画像貼り付け
- **CLAUDE.md チェッカー（独立ページ）** — 自分の `CLAUDE.md` を貼り付け / ドロップで 100 点満点採点（行数・キャッシュ効率・重複・構造）。問題点リストと、APIキーがあれば Haiku 経由で改善版もダウンロード可能。スコアも診断書画像として X にシェアできます
...
DashboardData { patterns, turns, toolBreakdown, ... }
  ...
  ├─ DiagnosisCard       → off-screen に SVG 混じりの診断書を生成
  │   └─ html2canvas     → PNG 化（ダウンロード or クリップボード）
  └─ /api/benchmark      → 集計指標のみ送信（オプトイン時のみ）

CLAUDE.md (paste / drop)
  ↓ claudeMdAnalyzer.ts  ← ブラウザ内
ClaudeMdScore (0-100)
  └─ haiku.ts            → 改善版生成（APIキーがあれば）
```
